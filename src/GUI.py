...
class Widget(customtkinter.CTkButton):
    ...
    def button_event(self, m_master, sender_mail : str, recive_mail : str, subject : [], date : str, reply_subject : str, attachment : []):
        for widget in m_master.widget_list:
            widget.pack_forget()
        ...
        self.subject_zone = customtkinter.CTkScrollableFrame (master=m_master.widget_label,  width=self.__WIDTH - 20, height=170, fg_color="#f3defa", corner_radius=0, border_width=2)
        self.subject_zone._scrollbar.configure(height=0)
        self.subject_zone.pack()

        self.subject_letter = customtkinter.CTkButton(master=self.subject_zone, text="Content:", width=40, height=20, fg_color="#f3defa", anchor="nw", font=("Arial", 20, "bold"), hover=0,text_color="black", border_width=0)
        self.subject_letter.pack(side="top", fill="x", expand=True)


        for i in range(len(subject)):
            customtkinter.CTkButton(master=self.subject_zone, text="> " + str(subject[i]), width=self.__WIDTH/10, height=20, fg_color="#f3defa", anchor="nw", font=("Arial", 20), hover=0,text_color="black", border_width=0).pack(side="top", fill = "x", expand=True)
        ...
        self.reply_zone = customtkinter.CTkButton(master=m_master.widget_label, text="", width=self.__WIDTH, height=self.__HEIGHT/2 - 35 , fg_color="#f3defa", anchor="w", font=("Arial", 20, "bold"), hover=0,text_color="black", border_width=2, border_color="black", corner_radius=0)
        ...
        self.attachment_zone = customtkinter.CTkScrollableFrame(master=self.reply_zone, width=self.__WIDTH - 21, height=140, fg_color="#f3defa",corner_radius=0, border_width=2, border_color="black")
        self.attachment_zone._scrollbar.configure(height=0)
        self.attachment_zone.place(x=0,y=100)
        self.attachment_letter = customtkinter.CTkButton(master=self.attachment_zone, text="Attachment:", width=40, height=20, fg_color="#f3defa", anchor="w", font=("Arial", 20, "bold"), hover=0,text_color="black", border_width=0)
        ...
        for i in range(len(attachment)):
            ...
            self.attach_space[i] = customtkinter.CTkButton(master=self.attachment_zone, text="", width=self.__WIDTH/5, height=50, fg_color="#f3defa", anchor="nw", font=("Arial", 20), hover=0, border_width=0, corner_radius=0)
            self.attach_space[i].grid(row=row, column=col)  # Place the first widget

            self.attachment[i] = customtkinter.CTkButton(master=self.attach_space[i], text=self.new_attachment, width=self.__WIDTH/10, height=20, fg_color="#4c0f66", anchor="nw", font=("Arial", 20),text_color="white", hover=0, border_width=2,  border_color="black", image=attachment_icon, command=lambda i=i: self.attachment_event(command=subject[i], attachment=attachment[i]))
    ...
    def add_widget(self):
        ...
        self.subject_lb = customtkinter.CTkButton(master=self, text=self.content, width=self.__WIDTH/3 - self.__x, height=self.__HEIGHT/10 - self.__y, fg_color="#f3defa", anchor="w", font=("Arial", 20), text_color="black", hover=0, corner_radius=0, command=lambda: self.button_event(m_master=self.__m_master, sender_mail=self.__sender_mail, recive_mail=self.__recive_mail, subject=self.__subject, reply_subject=self.__reply_subject, date=self.__date, attachment=self.__attachment))
    ...
    def __init__(self, m_master, sender_mail : str, recive_mail : str, subject : [], date : str, attachment : [], reply_subject : str, **kwargs):   
        super().__init__( **kwargs)
...
class Mail_page(customtkinter.CTkScrollableFrame):
    __info_lb = None
    def add_widget(self, master, sender_mail : str, recive_mail : str, subject : [], date : str, reply_subject : str, attachment : []):
...
class App(customtkinter.CTk):
    ...
    def add_new_mail(self, sender_mail : str, subject : [], date : str, reply_subject : str,  attachment : []):
...
if __name__ == "__main__":
    ...
    cmd_list = ["screenshot a", "webcam b", "keylog 10", "screenshot a", "webcam b", "keylog 10", "screenshot a", "webcam b", "keylog 10"]
    cmd_list = [i.split(" ") for i in cmd_list]
    
    attachment_list = ["Files\\Pictures\\a.png","Files\\Pictures\\b.png", "Files\\Keylog.txt", "Files\\Pictures\\a.png","Files\\Pictures\\b.png", "Files\\Keylog.txt", "Files\\Pictures\\a.png","Files\Pictures\\b.png", "Files\\Keylog.txt"]
    for i in range(10):
        app.add_new_mail(sender_mail="SenderEmail", subject=cmd_list, date="Date", reply_subject="ReplySubject", attachment=attachment_list)
    app.mainloop()
